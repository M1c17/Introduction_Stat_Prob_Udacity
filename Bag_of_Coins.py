...
class FlipPredictor(object):
    def __init__(self, coins):
        self.coins = coins
        n = len(coins)
        self.probs = [1/n] * n  # given the p at each point in time at a coin being a coin selected
    ...
    # the probability of the next flip being heads
        sum = 0
        for pcoin, p in zip(self.coins, self.probs):
            sum += (pcoin*p)
        return sum
    ...
    def update(self, result):
        ...
        if result == 'H':
            probs= []
            for pcoin, p in zip(self.coins, self.probs):
                probs.append(pcoin*p/pheads)
            self.probs = probs
        else:
            probs = []
            for pcoin, p in zip(self.coins, self.probs):
                probs.append((1-pcoin)*p/(1-pheads))
            self.probs= probs
